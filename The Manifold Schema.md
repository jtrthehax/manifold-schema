...
$\lambda$ is inferred from which hemisphere's $\Theta^*$ degrades first under load. It is not a permanent trait — it is the current configuration of the manifold's tilt. See Section 17 for working state profiles.
...
| ------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------- |
| **IRQ routing** — $I^*$               | Routes gain to signals — determines which signals the chain runs on                     | $R^* \rightarrow K$ — precision sets the noise floor, $I^*$ sets the gain            | §15     |
...
**Why depth matters more than rate:** Breath depth moves CO₂. Rate does not. A fast shallow breath depletes CO₂ tolerance and collapses the chain from the top. A slow deep exhale stabilises phase-locking and widens the window. Each link in the chain above is already confirmed by independent RSA, HRV, breath-hold, and hyperventilation literatures. See Section 9 for anchors.
...
| 14     | $I^*$ routes to a region → signal amplifies (gain control loop)                                      | Petzschner et al. (2018) — HEP study                                                          | Attention to heartbeat increases HEP amplitude and predicts autonomic reactivity — $I^*$ allocation amplifies the signal it routes to, not merely permits it                                                                                                                                                                   |
...
|---|---|---|---|
| $f_{routing}$ | $I^*_{total} - \sum_i P_i \cdot W_i$ | Remaining routing capacity after competing loads — visual, threat, prior-driven prediction | Saturated by competing load — capacity arithmetically removed |
| $f_{gain}$ | $\frac{1}{1 + e^{-k(G - G_0)}}$ | Sigmoid gain control — amplification of the signal being routed to | Gain collapses below threshold $G_0$ — signal present but not amplified |
| $f_{sensorium}$ | $I^*_{available} / I^*_{required}$ | Whether available routing meets task demand | Task demand exceeds available routing — signal readable but insufficient for the operation |
...
**The core change in v6.0 is mathematical completion. The framework's equations now do the work the prose was doing.**

**§2b — Master Equation**
- Replaced multiplicative product with weighted geometric mean — eliminates exponential decay artifact at healthy baselines
- Weights reflect empirical collapse sequence: $R^{*0.30}$, $W^{*0.25}$, $A_s^{*0.15}$, $\Theta^{*0.15}$
- $I^*$ removed from $C_s$ — routing layer formally separated from bandwidth equation

**§2c — Curvature Equation**
- Added $\epsilon$ floor — removes infinite curvature artifact as $R^* \rightarrow 0$
- Added transfer functions $W^* = \frac{1}{1 + \alpha K}$ and $\Theta^* = \frac{1}{1 + \beta K}$ — causal chain is now mathematically closed
- $W^*$ and $\Theta^*$ formally identified as derived variables — independent inputs are $A_s^*$, $R^*$, $L^*$, $\sum_i S_i \cdot C_i$

**§6a — Prior Update Rate**
- $dM/dt \rightarrow \mathcal{U}$ throughout — removes false calculus implication
- Functional form defined: $\mathcal{U} = \frac{A_s^* \cdot R^* \cdot \Theta^*}{1 + \gamma K_{enc}}$
- Feedback loop now computable — curvature contamination rule exits prose

**§14 — Interoception as Sensorium**
- $I^*$ decomposed into $f_{routing} \cdot f_{gain} \cdot f_{sensorium}$
- Three failure modes identified and separated — saturated routing, collapsed gain, insufficient sensorium
- Decomposition positioned as mechanistic elaboration — scalar $I^*$ retained in master equation, consistent with $K$ architecture

**§16 — AI Connection (removed)**
- Domain projection delegated to dedicated papers
- Pointer retained in domain projections table and §0
- Closing statement updated — AI references removed

**§18 — Closing Statement**
- Rewritten — closes on mechanistic statement rather than AI analogy

**Section renumbering**
- Former §17 (Social Projection) → §16
- Former §18 (Topology Complete) → §17
- Former §19 (Closing Statement) → §18
