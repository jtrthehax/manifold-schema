...
This changelog exists so that any reader — human or AI — can see the convergence path. Precision v3.4 was converged with Central Reference v1.4. The v3.4 → v3.5 delta integrates a new external finding (the pearled axon morphology) and completes a reference audit. The changes are:

| Step | Change                                                                                                                                   | Location        | Reason                                                                                                                                                                                                                                    | Source anchor          |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| 1    | Header updated to v3.5                                                                                                                   | Header          | Versioning track                                                                                                                                                                                                                          | —                      |
| 2    | Physical substrate of $D_T$ added — axon pearl geometry as the cellular implementation of timing distance                                | §1.1            | Griswold et al. (2024) demonstrated non-myelinated axons are strings of repeating pearls, not smooth tubes; pearl dimensions determine conduction velocity. This is the physical substrate the precision framework previously lacked.     | Griswold et al. (2024) |
| 3    | Activity-dependent reshaping of the substrate added — axon pearls enlarge under high-frequency stimulation, changing conduction velocity | §1.2            | Griswold et al. (2024): 8% longer, 17% wider after stimulation; effect lasting at least 30 minutes. Signal slowing persisted at least an hour. The axon is not a fixed cable — its geometry responds to activity.                         | Griswold et al. (2024) |
| 4    | Human tissue confirmation added — pearled axons found in human cortical tissue                                                           | §1.2            | Eddings et al. (2026) confirmed pearled architecture in human cortical tissue obtained during epilepsy surgery. Extends the finding beyond mouse models.                                                                                  | Eddings et al. (2026)  |
| 5    | Membrane mechanics as fifth precision interface added — cholesterol, tension, and fluidity affect pearl geometry                         | §1.4, §2.3      | Griswold et al. (2024): cholesterol removal alters pearl geometry and slows signaling; membrane tension changes pearl size. The membrane's physical state is a precision variable at the axon level, alongside the four CO₂ channels.     | Griswold et al. (2024) |
| 6    | Substrate dynamics added to temporal dynamics — encoding window $W_{enc}$ as the axon's pearl configuration at encoding                  | §4.6            | The geometry at encoding is not a statistical state — it is the axon's actual pearl configuration. The prior update $\mathcal{U}$ writes into the substrate the pearls instantiate.                                                       | Griswold et al. (2024) |
| 7    | Physical substrate rows added to variable mapping table                                                                                  | §1.10           | Timing distance $D_T$, activity-dependent precision reshaping, and membrane mechanics mapped to their physical substrates.                                                                                                                | Griswold et al. (2024) |
| 8    | PREDICT-PREC-11 added — axon pearl geometry tracks precision                                                                             | Part VII        | Falsifiable prediction: pearl geometry correlates with $P$ at EEG and HRV layers; membrane modulation changes $P$ before CO₂ or HRV changes.                                                                                              | Griswold et al. (2024) |
| 9    | Overview sentence added — precision variable is not an abstraction, it is the axon's shape                                               | Overview        | Frames the new substrate integration for readers encountering the paper for the first time.                                                                                                                                               | Griswold et al. (2024) |
| 10   | Reference audit completed — all body-text citations inventoried                                                                          | References      | Reference section was lost in a prior iteration when AI dropped the section assuming it persisted. Audit identifies 11 external citations (9 pre-existing, 2 new) plus framework papers requiring restoration.                            | —                      |
| 11   | References section flagged for restoration                                                                                               | References      | List of body-text citations provided; external references must be recovered from earlier draft or vault. Framework papers (Manifold Schema v7.2, Geometry of Inference v1.0, Central Reference v1.4, Allostatic Load v2.1) must be added. | —                      |
| 12   | Version history updated                                                                                                                  | Version History | —                                                                                                                                                                                                                                         | —                      |
| 13   | Changelog added at top                                                                                                                   | This section    | Versioning track                                                                                                                                                                                                                          | —                      |

**What did not change:** The mechanisms. The precision equation, the CO₂ tolerance window, the two-factor pressure, the chemoreflex jitter, the collapse hysteresis, the resonance breathing, the cross-frequency coupling, the motor gain formalization, the temporal dynamics, the encoding window, and the $P$/$P_{eff}$ distinction — every mechanism is unchanged from v3.4. The v3.5 additions are the **physical substrate layer** at the cellular level, plus the reference audit. The substrate was always implicit in the framework (the manifold runs on physical tissue), but the pearled axon finding makes it explicit and measurable.

**Note on the reference loss:** The references section was lost in a prior iteration when the AI produced a confident statement ("the references section stays the same") that treated the section as a cached pattern rather than a live dependency. The section was not carried forward. The framework's own prediction describes this failure: the AI's decompression topology classified the references as known and stable, and the classification was wrong. The correction: the references section must be carried verbatim in every future iteration. The AI should be told explicitly not to treat it as cached.
...
The pearled axon finding (Griswold et al., 2024) adds a physical substrate at the cellular level. The axon's pearl geometry determines the conduction velocity. The conduction velocity determines the timing. The timing determines $P$. The precision variable is not an abstraction — it is the axon's shape.
...
**Physical substrate of $D_T$.** The timing distance $D_T$ is not an abstract property of two oscillatory streams. It has a physical substrate in the axon's morphology. Griswold et al. (2024) demonstrated that non-myelinated axons are not smooth tubes but **strings of repeating pearls** — non-synaptic varicosities whose dimensions are determined by membrane mechanics (tension, fluidity, cholesterol content). The pearls' size determines the conduction velocity: wider regions allow ions to pass more quickly, narrower regions create resistance. The axon's shape is therefore the physical implementation of the timing geometry that $D_T$ measures. The membrane mechanics — not an internal cytoskeletal scaffold — determine the pearl dimensions (the study tested the cytoskeletal hypothesis and found that disrupting the scaffold did not eliminate the pearls).
...
**Activity-dependent reshaping of the substrate.** Griswold et al. (2024) found that high-frequency electrical stimulation caused the pearls to become 8% longer and 17% wider, with the enlargement lasting at least 30 minutes. Electrical signaling slowed for at least an hour after stimulation. The axon is not a fixed cable — its geometry responds to activity, and the response changes the timing. This is the physical mechanism by which prior activity reshapes the substrate for subsequent precision. The timing geometry is not static. It is continuously reconfigured by the loop's own operation. A follow-up study (Eddings et al., 2026) confirmed the pearled architecture in **human cortical tissue** obtained during epilepsy surgery, extending the finding beyond mouse models.
 ...
 **Membrane mechanics as the physical interface.** The precision framework has specified the CO₂ mechanism (Bohr effect, vasodilation, neural firing thresholds, KCC2 function). The pearled axon finding adds the **membrane** as a fifth interface. Cholesterol removal makes the membrane more fluid, alters the pearl geometry, and slows electrical signaling. Membrane tension changes the pearl size. The membrane's physical state is a precision variable at the axon level. This suggests that the four CO₂ channels and the membrane mechanics are not independent — they are the same substrate viewed at different scales. The CO₂ channels determine the substrate's energy availability. The membrane determines the substrate's timing geometry. Both feed into $P$.
...
**Substrate dynamics.** The temporal dynamics of $P$ have a physical substrate in the axon's activity-dependent reshaping. When the loop runs, the axon's pearls reshape. The reshaping changes the conduction velocity. The conduction velocity changes the timing. The timing changes $P$. This is the physical mechanism behind the encoding window $W_{enc}$: the geometry at encoding is not just a statistical state — it is the axon's actual pearl configuration at the moment of encoding. The prior update $\mathcal{U}$ writes into the substrate that the pearls instantiate.
...
**PREDICT-PREC-11 — Axon Pearl Geometry Tracks Precision**

> The pearl geometry of non-myelinated axons will correlate with the precision variable $P$ measured at the EEG and HRV layers. Higher precision states will show systematically different pearl dimensions than lower precision states. Membrane modulation (cholesterol, osmotic pressure) that alters pearl geometry will produce measurable changes in $P$ before any change in CO₂ or HRV.

| Field | Content |
|---|---|
| Test | Measure pearl geometry via high-pressure freezing EM or live imaging in parallel with EEG phase coherence, HRV, and capnometry. Modulate membrane mechanics and measure the change in $P$. |
| Outcome if confirmed | The pearl geometry is the physical substrate of $P$ — the precision variable has a measurable cellular implementation. |
| Outcome if disconfirmed | Pearl geometry does not correlate with $P$ — the timing-coherence ratio is not physically instantiated in the axon's shape. |
| Status | Untested — requires high-pressure freezing EM and simultaneous electrophysiology. |
...
| ------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v3.5    | 2026-09-17 | - Pearled axon finding integrated as the physical substrate of $D_T$ (§1.1, §1.2, §1.4, §4)<br>- PREDICT-PREC-11 added<br>- Reference audit completed<br>- References section restored (or flagged for restoration if not yet recovered)                                                                                                    |
...
| Citation                | Section                                  | Context                                                                                     |
| ----------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------- |
| Zelano et al., 2016     | Overview, §1.9, Part V                   | Nasal respiration entrains limbic oscillations                                              |
| Raichle & Plum, 1972    | Overview, §1.4, §1.4a, Part V            | CO₂ drives cerebral vasodilation; hyperventilation produces vasoconstriction                |
| Meuret & Ritz, 2010     | Overview, §1.4a, Part V                  | Hypocapnia produces cognitive and anxiety collapse; panic disorder characterized by low CO₂ |
| Zaccaro et al., 2018    | Overview, §1.3, §1.8, Part V             | Slow breathing produces HRV and EEG coherence; frequency-dependent                          |
| Pratap et al., 2026     | Overview, §1.1, Part V                   | Cardiorespiratory coupling during encoding predicts cognitive performance                   |
| Yamamoto et al., 2006   | Overview, §1.9, Part V                   | Zen meditation increases interhemispheric EEG coherence; anxiety negatively correlated      |
| Cooper et al., 2003     | Overview, §1.3, Part V                   | Control Pause operationalizes $C_{high}$; Buteyko trial                                     |
| Kox et al., 2014        | Overview, §1.3, PREDICT-PREC-09, Part V  | Wim Hof breathing training shifts autonomic parameters                                      |
| Sakakibara et al., 1994 | §2.1                                     | Voluntary apnea produces decreased theta waves; hypercapnia depresses cortical activity     |
| Xue et al., 2026        | §1.11                                    | Feature interference as $K$ rising; V1 entanglement under task uncertainty                  |
| Griswold et al., 2024   | **NEW** — §1.1, §1.2, §1.4, §4, Overview | Pearled axon finding — membrane mechanics, activity-dependent reshaping                     |
| Eddings et al., 2026    | **NEW** — §1.2                           | Pearled axons in human cortical tissue                                                      |*
